...
// --- Video.js helper (optional) ---
function getVjs(){
  var el = document.getElementById('videoPlayer');
  if (!window.videojs || !el) return null;
  if (window.currentVjs && !window.currentVjs.isDisposed()) return window.currentVjs;
  try {
    window.currentVjs = window.videojs(el, {
      autoplay: true, controls: true, preload: 'auto', fluid: true, playsinline: true
    });
    return window.currentVjs;
  } catch(e){ console.warn('[IPTV] videojs init failed', e); return null; }
}
...
function resetPlayers(){
  ...
  try { if (window.currentDash){ window.currentDash.reset();  window.currentDash = null; } } catch(e){}  try { if (window.currentVjs && !window.currentVjs.isDisposed()) { window.currentVjs.dispose(); window.currentVjs = null; } } catch(e){}
...
function playByType(url){
  ...
  if (isMP4 && v){
    ...
    var pv = getVjs();
    if (pv){
      try { pv.src({ src: u, type: 'video/mp4' }); pv.play().catch(()=>{}); } catch(_){}
      showPlaying(); return;
    }
    v.src = u;
  ...
  if (isHLS && v){
    ...
    var pv = getVjs();
    if (pv){
      try { pv.src({ src: u, type: 'application/x-mpegURL' }); pv.play().catch(()=>{}); } catch(_){}
      showPlaying(); return;
    }
    // fallback to Hls.js if available
    if (window.Hls && window.Hls.isSupported()){
      try {
        ...
        hls.loadSource(u);
        hls.attachMedia(v);
        hls.on(Hls.Events.MANIFEST_PARSED, () => { try { v.play().catch(()=>{}); } catch(_){} });
      } catch(_){}
      showPlaying(); return;
    }
    // ultimate fallback: native
    try { v.src = u; v.play().catch(()=>{}); } catch(_){}
    showPlaying(); return;
  }
